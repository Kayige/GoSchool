...
func createDoubleLinkList() *dblinklist {
	dblist := &dblinklist{}
	dblist.addNode("Mary")
	dblist.addNode("Joe")
	dblist.addNode("David")
	dblist.addAtPos(3, "NewNode")
	return dblist
}

func printContent(p *dblinklist) {
	p.printAllNodes()
}

func printReverseContent(p *dblinklist) {
	p.printAllNodesReverse()
}

func removeAt(p *dblinklist, i int) {
	p.remove(i)
}

func main() {
	myDoubleList := createDoubleLinkList()
	printContent(myDoubleList)
	fmt.Println()
	printReverseContent(myDoubleList)
	removeAt(myDoubleList, 3)
	fmt.Println()
	printContent(myDoubleList)
